...
//1.配置表单
class TestForm extends \util\form\Form {
        ...
        parent::__construct(array(
            'im' => array(
                'rules' => array(
                    ...
                    array('regexp', \util\form\validator\RegexpConfig::QQ, 'im格式有误!'),
                )
            ),
            'title' => array(
                'rules' => array(
                    array('required', 'need_check_title', '必填写'),
                    array('maxLength', 100, '最长不超过100位'),
                    array('minLength', 15, '最短不低于15'),
                    array('regexp', \util\form\validator\RegexpConfig::NO_SPECIAL, 'regexp description', true),
                    array('regexp', \util\form\validator\RegexpConfig::INCLUDE_PHONE, '标题不能出现电话号码哦', true),
...
<body>
<form>
    <input name='title' id="title" data-rules="<?php echo $this->helper('get_data_rules', array('form' => $form, 'name' => 'title'));?>" />
    <input name='username' id="username" data-rules="<?php echo $this->helper('get_data_rules', array('form' => $form, 'name' => 'username'));?>" />
    <input name='password' id="password" data-rules="<?php echo $this->helper('get_data_rules', array('form' => $form, 'name' => 'password'));?>" />
