...
class Field {
    ...
    public function __construct($fieldName, $fieldConfig, $form) {
        if(count($fieldConfig) < 1) {
            throw new \CException("rule must be needed");
        }
        ...
        //form
        if (!($form instanceof \util\form\Form)) {
            throw new \CException("form must be instance of Form.php");
        }
        $this->form = $form;
    ...
    public function addRule($rule) {
        if(!is_array($rule) || count($rule) < 2) {
            throw new \CException("the first args must be a array");
        }
        $this->rules[] = $rule;
    }
    ...
    public function validate($value) {
        ...
        foreach ($rules as &$ruleInfo) {
            if ($ruleInfo[0] == 'required') {
                ...
                if (is_string($ruleInfo[1])) {
                    ...
                    $exp = new \util\form\expression\Expression($ruleInfo[1], $this->form, $fieldValue);
        ...
        $fieldValidator = new \util\form\validator\FieldValidator($fieldConfig, $this->form);
